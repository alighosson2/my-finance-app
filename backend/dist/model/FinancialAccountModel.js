...
class FinancialAccountEntity {
    ...
    balance, currency, bank_name, account_number_masked, is_active, created_at, updated_at) {
        this.id = id;
    ...
    static fromPrisma(account) {
        return new FinancialAccountEntity(account.id, account.user_id, account.bank_token_id || null, account.account_name, account.account_type, Number(account.balance), account.currency, account.bank_name, account.account_number_masked, account.is_active ?? true, account.created_at ?? new Date(), account.updated_at ?? new Date());
    }
    toJSON() {
        return {
            id: this.id,
